 ...
 *
 */
class ComponentArchitectViewComponents extends JViewLegacy
{
	protected $items;
	...
	protected function addToolbar()
	{
		...
		JToolbarHelper::title(JText::_('COM_COMPONENTARCHITECT_COMPONENTS_LIST_HEADER'), 'stack components');
		...
		if (version_compare(JVERSION, '3.0', 'lt'))
		{
			...
			$url = 'index.php?option=com_componentarchitect&view=generatedialog';
			$bar->appendButton( 'Link', 'generate', JText::_('COM_COMPONENTARCHITECT_GENERATE_BUTTON'), $url);

			$url = 'index.php?option=com_componentarchitect&view=componentwizard';
		...
		if (version_compare(JVERSION, '3.0', 'ge'))
		{
			JToolBarHelper::custom('generatedialog.display', 'cogs', '', JText::_('COM_COMPONENTARCHITECT_GENERATE_BUTTON'), false);		
		}	
		//[%%END_CUSTOM_CODE%%]
	}
	/**
	...
	{	
		...
		JHtmlSidebar::setAction('index.php?option=com_componentarchitect&view=components');
