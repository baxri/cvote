 ...
 *
 */
class GeocarsTableCars extends JTable
{
	var $state				= 1;
	/**
	 * Constructor
	...
	function __construct(&$_db)
	{
		...
		parent::__construct('#__geocars_cars', 'id', $_db);
	...
	function check()
	{
		...
		$table = JTable::getInstance('Cars', 'GeocarsTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_GEOCARS_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
