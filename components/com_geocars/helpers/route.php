...
/**
 * Cars In Georgia Component Route Helper
 *
 */
abstract class GeocarsHelperRoute
{
	protected static $lookup;
	/**
	 * @param	int	The route of the Car
	 */
	public static function getCarRoute($id, $cat_id = 0, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		$needles = array(
			'car'  => array((int) $id)
		);
		...
		{
			//Create the link
			$link = 'index.php?option=com_geocars&view=car&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_geocars&view=car&layout='.$layout.'&id='. $id;
		}
	...
	public static function getCategoryRoute($cat_id, $keep_item_id = false, $language = 0)
	{
		...
		else
		{
			...
			$options['countItems'] = false;
			$options['table'] = '';			
			$category = JCategories::getInstance('Geocars', $options)->get($id);
		}
		...
		else
		{
			...
			//Create the link
			$link = 'index.php?option=com_geocars&view=category&id='.$id;
	...
	protected static function _findItem($needles = null, $keep_item_id = false, $layout = 'default')
	{
		...
		if ($active AND $active->component == 'com_geocars' AND ($language == '*' OR in_array($active->language, array('*', $language)) OR !JLanguageMultilang::isEnabled()))
		{
			return $active->id;
		}
