...
/**
 * HTML View class for the Cars In Georgia component
 *
 */
class GeocarsViewCategory extends JView
{
	protected $state;
	protected $category;
	protected $children;
	...
	protected function _prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_GEOCARS_CATEGORY_HEADER'));
		}

		$id = (int) @$menu->query['id'];

		if ($menu AND ($menu->query['option'] != 'com_geocars' OR
					 $menu->query['view'] == 'car' OR 
					 $id != $this->category->id))
		{
			$path = array(array('title' => $this->category->title, 'link' => ''));
			$category = $this->category->getParent();

			while (($menu->query['option'] != 'com_geocars' OR
					$menu->query['view'] == 'car' OR
		...
		$this->document->addStyleSheet(JUri::root()."components/com_geocars/assets/css/".JString::strtolower(str_replace(' ','',$this->params->get('items_to_display'))).".css");
		...
		if ($lang->isRTL())
		{
			...
			$this->document->addStyleSheet(JUri::root()."components/com_geocars/assets/css/".JString::strtolower(str_replace(' ','',$this->params->get('items_to_display')))."-rtl.css");
