 ...
 * @package			com_geocars
 * @subpackage		com_geocars.itemnavigation
 * @copyright		
...
/**
 * Geocars navigation plugin class.
 *
 */
class plgGeocarsItemnavigation extends JPlugin
{
	/**
	 * Constructor
	 *
	...
	public function onCarBeforeDisplay ($context, &$row, &$params, $page=0)
	{
		...
		if ($params->get('show_car_navigation') AND 
			($context == 'com_geocars.car') AND 
			($view == 'car')) 
		{
			...
			// Location of current car item in array list.
			...
			{
				$row->prev = $location -1 ; 
				// The previous car item cannot be in the array position -1.
			...
			{
				$row->next = $location +1;
				// The next car item cannot be in an array position greater than the number of array postions.
			...
			$keep_item_id = (int) $params->get('keep_car_itemid', 0);		
					
			if ($row->prev) 
			{
				$row->prev = JRoute::_(GeocarsHelperRoute::getCarRoute($row->prev->slug,$row->prev->catslug, $row->prev->language, $layout, $keep_item_id));
			} 
			else 
			{
				$row->prev = '';
			...
			if ($row->next) 
			{
				$row->next = JRoute::_(GeocarsHelperRoute::getCarRoute($row->next->slug,$row->next->catslug, $row->next->language, $layout, $keep_item_id));
			...
			if ($row->prev OR $row->next) 
			{
				...
				$row->pagination = $html;
				$row->paginationposition = $this->params->get('car_position', 1);
				// This will default to the 1.5 and 1.6-1.7 behavior.
				$row->paginationrelative = $this->params->get('car_relative',0);				
			}
		}
