...
class GeocarsController extends JController
{
	...
	public function display($cachable = false, $url_params = false)
	{
		...
		// Load the submenu.
		GeocarsHelper::addSubmenu($view);

		switch ($view)
		{
			case 'car': 
			case 'cars':
				require_once JPATH_COMPONENT.'/helpers/cars.php';			
				break;
		...
		{
			case 'car': 
				if ($layout == 'edit' AND !$this->checkEditId('com_geocars.edit.car', $id))
				{

					// Somehow the person just went to the form - we don't allow that.
					$this->setError(JText::sprintf('JLIB_APPLICATION_ERROR_UNHELD_ID', $id));
					$this->setMessage($this->getError(), 'error');
					$this->setRedirect(JRoute::_('index.php?option=com_geocars&view=cars', false));
