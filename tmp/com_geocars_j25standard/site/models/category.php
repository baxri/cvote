...
class GeocarsModelCategory extends JModelList
{
	/**
	 * Category items data
	 *
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$this->setState('list.car_filter', JRequest::getString('car-filter-search'));
		...
		{
			$limit = $app->getUserStateFromRequest('com_geocars.category.list.' . $item_id . '.limit', 'limit', $app->getCfg('list_limit'));
		}
	...
	public function getItems()
	{
		...
		if ($params->get('items_to_display') AND $params->get('items_to_display') !='None')
		{
			...
			{
				$model = JModel::getInstance($object_upper_case, 'GeocarsModel', array('ignore_request' => true));
	...
	protected function _buildOrderBy()
	{
		...
		$item_id		= JRequest::getInt('id', 0) . ':' . JRequest::getInt('Itemid', 0);
		$order_col	= $app->getUserStateFromRequest('com_geocars.category.list.' . $item_id . '.filter_order', 'filter_order', '', 'string');
		$order_dirn	= $app->getUserStateFromRequest('com_geocars.category.list.' . $item_id . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
		$order_by	= ' ';
		...
		{
			case 'Cars' :
				$secondary_order_by	= $params->get('car_orderby_sec', 'none');
				$order_date			= $params->get('car_order_date');
				$category_order_by	= $params->def('car_orderby_pri', '');
				
				$primary			= GeocarsHelperQuery::orderbyPrimary($category_order_by);	
				$secondary			= GeocarsHelperQuery::orderbySecondary($secondary_order_by, $order_date, 'ordering');
	...
	public function getCategory()
	{
		if(!is_object($this->_item))
		{
			...
			if ($params->get('items_to_display') AND $params->get('items_to_display') !='None')
			{
				$options['table'] = '#__geocars_'.JString::strtolower(str_replace(' ','',$params->get('items_to_display')));
			}
			else
			{
				$options['table'] = '';
			...
			if(is_object($this->_item))
			{
				$user	= JFactory::getUser();
				$user_id	= $user->get('id');
				$asset	= 'com_geocars.category.'.$this->_item->id;
